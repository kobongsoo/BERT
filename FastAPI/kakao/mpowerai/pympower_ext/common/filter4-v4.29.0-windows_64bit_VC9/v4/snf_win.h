...
/**
 * thmx 파일 내용을 버퍼에 필터링한다.
 * @param	pMFI thmx 파일이 담긴 메모리 포인터
 * @param	pBuf 추출한 텍스트가 저장될 SN3BUF 포인터
 * @param	WithPage  페이지 구분자 출력 여부. 1이면 출력, 0이면 출력안함.
 * @return	제대로 필터링해서 버퍼에 쓰면 SN3OK 를 반환하고
			오류가 발생하면 해당 오류코드를 반환한다.
...
/**
 * ppam 파일 내용을 버퍼에 필터링한다.
 * @param	pMFI ppam 파일이 담긴 메모리 포인터
 * @param	pBuf 추출한 텍스트가 저장될 SN3BUF 포인터
 * @param	WithPage  페이지 구분자 출력 여부. 1이면 출력, 0이면 출력안함.
 * @return	제대로 필터링해서 버퍼에 쓰면 SN3OK 를 반환하고
			오류가 발생하면 해당 오류코드를 반환한다.
...
* @return	성공 시 SN3OK를, 실패 시 오류코드를 반환한다.
*/
SNF_WIN_API int FAR PASCAL wsnf_egg_getfile_m(SN3MFI *pMFI, SN3MFI *pUzFile, __uint8* pFileName);
...
/**
 * xml 파일 내용을 버퍼에 필터링한다.
 * @param	pMFI xml 파일이 담긴 메모리 포인터
 * @param	pBuf 추출한 텍스트가 저장될 SN3BUF 포인터
 * @return	제대로 필터링해서 버퍼에 쓰면 SN3OK 를 반환하고
			오류가 발생하면 해당 오류코드를 반환한다.
 */
